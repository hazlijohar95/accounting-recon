...
// The component must be generated by running `npx convex dev`
const workOSComponent = components?.workOSAuthKit;

if (!workOSComponent) {
  console.error('[AuthKit] workOSAuthKit component not found. Run `npx convex dev` to generate types.');
  // Throw in production, warn in development
  if (process.env.NODE_ENV === 'production') {
    throw new Error('WorkOS AuthKit component not initialized. Run `npx convex dev` first.');
  }
}

export const authKit = new AuthKit<DataModel>(
  workOSComponent ?? ({} as never), // Never used if component is missing - will throw above in prod
  {
    authFunctions,
  }
);
